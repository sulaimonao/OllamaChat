...
from .core import Crawler, Index, seed_urls_from_sitemap, seed_urls_from_file
...
def crawl(urls: Optional[str] = typer.Option(None, help="Comma-separated URLs to seed"),
          ...
          db: str = typer.Option("termsearch.db", help="SQLite index path"),
...
@app.command()
def rank(db: str = typer.Option("termsearch.db"), iters: int = typer.Option(30), damping: float = typer.Option(0.85)):
    idx = Index(db)
...
@app.command()
def show(doc_id: int, db: str = typer.Option("termsearch.db")):
    idx = Index(db)
    doc = idx.get(doc_id)
    if not doc:
...
@app.command()
def export(out: str = typer.Option("docs.jsonl"), db: str = typer.Option("termsearch.db")):
    idx = Index(db)
    count = 0
